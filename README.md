# java-lotto-precourse
이 프로젝트는 구입한 로또의 당첨 내역과 수익률을 계산하는 로또 발매기를 구현합니다.

입출력은 콘솔을 통해 이루어집니다.

## 프로젝트 구조
```markdown
src/main/java
└── lotto
    ├── Application.java
    ├── LottoGame.java
    ├── constant
    │   ├── LottoConstant.java
    │   └── exception
    │       └── ErrorMessage.java
    ├── domain
    │   ├── lotto
    │   │   ├── Lotto.java
    │   │   ├── LottoNumber.java
    │   │   └── LottoVendingMachine.java
    │   └── winning
    │       ├── WinningInformation.java
    │       ├── WinningLotto.java
    │       ├── WinningStatisticDto.java
    │       └── WinningStatisticsCalculator.java
    └── view
        ├── InputView.java
        └── OutputView.java

```

## 기능 목록
- [x] 1개의 로또에는 6개의 숫자가 포함된다.
- [x] 1개의 로또에는 중복되는 로또 번호가 존재하면 안 된다.
- [x] 로또 1개의 가격은 1,000원이다.

- [x] 로또 번호는 범위(1~45 사이의 경계값을 포함한 정수) 내에서만 존재한다.
- [x] 서로 다른 두 로또가 가지는 로또 번호가 완전히 동일하더라도, 두 로또는 다르게 취급한다.

- [x] 로또 구입 금액을 입력 받는다.
  - [x] 구입 금액이 1,000원으로 나누어 떨어지지 않는다면 예외 처리한다.
  - [x] 로또 구입 금액만큼 로또를 발행한다.
  - [x] 발행한 로또 수량 및 번호를 출력한다.
    - [x] 이때 로또 번호는 오름차순으로 정렬된다.
- [x] 당첨 번호를 입력 받는다.
  - [x] 입력된 당첨 번호 사이에 중복이 존재하면 안 된다. 존재하면 예외 처리한다.
- [x] 보너스 번호를 입력 받는다.
  - [x] 보너스 번호와 당첨 번호 사이에 중복이 존재하면 안 된다. 존재하면 예외 처리한다.
- [x] 사용자가 구매한 로또 번호와 당첨 번호를 비교하여 당첨 내역 및 수익률을 출력한다. 출력을 마치면 로또 게임을 종료한다.
  - [x] 수익률은 소수점 둘째 자리에서 반올림한다.
- [x] 예외 상황 시 다음과 같이 "\[ERROR\]"로 시작하는 에러 문구를 출력한다.

## 설계
### 로또 번호에 중복된 숫자가 있는지 검증
#### 방법1. Set을 이용한 가장 간단한 구현
주어진 `LottoTest` 중 실패하는 테스트를 통과시키기 위해 그 구현을 고민했을 때,
직관적으로 떠오른 방식은 중복을 허용하지 않는 `Set` 타입으로 변환했을 때의 사이즈와 비교하는 것이다(아래 코드 참고).
```java
// 방법1
Set<Integer> set = Set.copyOf(numbers);
if (set.size() != numbers.size()) {
    throw new IllegalArgumentException("[ERROR] 로또 번호에 중복이 존재합니다.");
}
```

#### 방법2. Set을 사용하지만 더 빠르게 예외 발생시키기
물론 위 구현('방법1')은 중복 검증 테스트를 성공적으로 통과시킨다.

하지만 "스트림 연산을 통해서도 해결할 수 있지 않을까?"하는 의문이 들었고,
`how to find duplicated value using stream java`로 서칭했다.

그렇게 [처음 찾은 글](https://stackoverflow.com/questions/68656381/how-to-find-duplicate-elements-in-a-stream-in-java)에서는
스트림이 아니라 (내가 생각한 방법1과 유사하게) `Set`을 사용하는 방식을 제시했다.

하지만 그 효율성 측면에서 차이가 있는데, 일단 해당 코드를 보자.
```java
// 방법2
Set<Integer> set = new HashSet<>();
for (Integer number : numbers) {
    if (!set.add(number)) {
        throw new IllegalArgumentException("[ERROR] 로또 번호에 중복이 존재합니다.");
    }
}
```

차이를 알겠는가? 방법1의 경우에는 일단 주어진 `numbers` ***전체를 순회***한 뒤에야 `size`를 통해 중복을 판단한다.

하지만 방법2의 경우 각 요소를 `Set`에 `add`하면서 **_`false`가 리턴되는 순간 중복으로 판단_**한다.
즉, 얼리 리턴처럼 조건을 만족하자마자 빠르게 종료되기 때문에 중복이 존재하지 않거나 중복이 끝에만 존재하는 경우를 제외하고는 더 우수한 성능이 기대된다.
('기대된다'고 표현한 것은, `Set.copyOf` 메서드가 극한으로 최적화 되어 있어서 전체를 순회하지만, 단순한 순회 구현보다 훨씬 빠르게 동작하지 않을까? 하는 생각도 들기 때문이다)

#### 방법3. 스트림을 이용해서 중복 여부 뿐만 아니라, 구체적인 중복 값까지 파악하기
추가적으로 생긴 욕심은 '에러가 발생한 지점을 명확히 해주고 싶다!'는 것이었다.

예를 들어 2주 차에서는 예외가 발생했을 때의 입력값 전체(`numbers`)를 메시지에 포함해줬지만,
이번에는 그 중에서 실제로 예외의 원인이 되는 입력(중복된 숫자)만을 메시지에 포함해주고 싶다는 것이다.

이에 대한 해결은 [이 글](https://www.baeldung.com/java-list-find-duplicates#1-using-filter-and-setadd-method)을 참고했다.

예외의 원인이 되는 값 전체를 출력해주려면 결국 방법2처럼 빠른 예외를 발생시키지 못하고, 방법1처럼 전체를 순회해야 된다는 trade-off가 생기기는 한다.

하지만 해당 미션에서의 도메인 룰이 '로또 번호는 6개'여야 함을 포함하고 있고, 구현에서 검증하기 때문에 결국 전체 순회로 인한 비효율이 유의미한 수준이 되지는 않음이 보장된다.

전체 순회로 인해서 비효율이 생기려면 그 크기가 매우 커져야 하는데, 크기가 너무 큰 경우는 이미 로또 번호 개수에 대한 검증에서 예외 처리되기 때문이다.

따라서 다음과 같은 구현을 채택한다.
```java
// 방법3
Set<Integer> elements = new HashSet<>();
List<Integer> list = numbers.stream()
        .filter(n -> !elements.add(n))
        .toList();
```

#### 방법4. 함수형 프로그래밍은 side effect 지양한다.
다만, 방법3처럼 스트림이 외부 컨텍스트에 변화를 주거나(side effect) 의존하는 것은 함수형 프로그래밍의 지향점이 아님을 알고 있다.

내가 의도하는 구현에서는 `중복된 값들`만 있으면 되지, `중복 없애고 남은 값들`까지는 필요하지 않기 때문에 다음과 같이 스트림을 사용하지 않고 구현하는 편이 좋겠다.

(만약 `중복 없애고 남은 값들`도 필요한 경우라면 스트림의 `groupingBy`를 활용하면 side effect도 해소하고, 요구 사항 만족도 가능한 것으로 파악된다.)

```java
// 방법4
Set<Integer> set = new HashSet<>();
boolean duplicated = false;
for (Integer number : numbers) {
    if (!set.add(number)) {
        duplicated = true;
    }
}
if (duplicated) {
    throw new IllegalArgumentException("[ERROR] 로또 번호에 중복이 존재합니다.");
}
```

### 로또 번호를 LottoNumber Value Object로 설계
기존에는 생각하지 못했던 방법인데, [해당 PR](https://github.com/woowacourse-precourse/java-racingcar-8/pull/169/files#r2470659778)을
리뷰 중에 Value Object라는 개념에 대해 알게 되었고, 이번 미션에서 '로또 번호'에 대한 구현으로 적절하겠다고 판단했다.

주어진 기능 요구 사항 중 "로또 번호의 숫자 범위는 1~45까지이다."라는 요구 사항이 있다.

V.O.를 알지 못하던 때였다면 당연스레 `Lotto` 내부에서 범위 검증을 했겠으나, 이제는 다르다!

`Lotto`는 어떤 규칙(개수 제한, 중복 금지 등)을 기반으로 `로또 번호`들을 가지는 객체이다. 그 자체로 `로또 번호`가 아니다. 따라서 로또 번호 도메인에 대해서 V.O.로 만들어서 관리함이 적절하다.

#### 고민1: 로또 번호 45개를 미리 만들어 둘 필요가 있을까?
결론: **YES**

만약 사람 A와 B 모두 `2`라는 로또 번호를 가지고 있다고 하자. 이 둘을 다르다고 할 수 있을까?

내 답은 "아니다"이다. 로또 번호를 누가(A, B) 소유했는지는 로또 번호가 알 바가 아니다. 로또 번호는 단순히 로또 번호일 뿐이다.

당첨 번호 중 하나가 `2`일 때 A의 `2`는 당첨이고, B의 `2`는 당첨이 아니라면 논리적 결함이 있지 않은가?

로또 번호 `2`는 그 어떤 상황에서도 동일한 `2`로 취급받아야 한다. 즉, 클래스 레벨에서 공유되도록 구현함이 적절하다.

#### 로또, 로또 번호와 정렬
'오름차순으로 정렬되는' 책임은 출력단에서 가지는 것이 아니라고 판단한다.

현실 세계의 로또에 근거했을 때 단순히 출력단에서만 아닌, 로또 자체에서 로또 번호를 정렬해서 가질 책임을 가진다고 생각함이 그 이유다.

따라서 `LottoNumber` 스스로 정렬 기준을 가지고(미션의 경우 오름차순), `Lotto`는 "로또 번호야 정렬되거라"라고 선언적으로 명령만 하는 것이 정렬 책임을 가지면서도
로또 번호의 자율성을 보장하는 방법이라고 생각한다.

### 프로그램 흐름 설계
사진 내용 중 일부: ~~입출력 시나리오상 구매한 복권 번호들이 공개된 상태에서 당첨 번호를 입력하게 하면 뭔가 이상하다. 따라서 출력은 제일 나중에 몰아서 한다.~~

![프로그램-흐름-설계](https://github.com/user-attachments/assets/af411f1b-ba22-472e-8b35-c053e4040e03)

#### 출력 시점에 대한 생각 변화
위 결정은 주어진 실행 결과 예시를 당시에 제대로 보지 않고 내렸던 것으로 판단된다.

구매 내역을 출력 후, 당첨 번호 입력을 받는 것이 적절하다.

또한 '구매한 로또의 발행' 책임은 `LottoVendingMachine`가 가지도록 했지만, 발행 이후의 '출력' 책임에 대해서는 별도의 출력 전용 객체가 가지는 것이 적절하다.

출력은 단순히 UI적인 요소이기 때문에, 그 요구가 자주 변경될 가능성이 있지만, 애플리케이션의 핵심 로직인 '로또 발행'은 그럴 확률이 적고, 또한 그럴 확률이 높아서도 안 되기 때문에 도메인 자체의 책임과 출력 책임을 구분하려는 의도다.

### Lotto 객체는 equals를 재정의 하지 않는다.
서로 다른 `Lotto` 객체의 내부 `numbers` 값들이 모두 동일하더라도 그 둘은 '동등하지 않은 객체'로 판단함이 옳다.

왜냐하면 번호 6개가 완전히 동일한 두 `Lotto`가 우연히 발행되었다고 하자. 그리고 두 로또는 당첨 번호와 3개 일치했다고 하자.

위 상황에서 '3개 일치한 로또의 개수'는 1개이면 안 된다. 당연히 2개여야 한다.

그런데 이펙티브 자바의 내용을 보면 `equals`를 논리적 비교로 재정의하는 것이 사실상 표준으로 여겨지는 것 같은데,
`Lotto`가 이를 재정의 하지 않으면 다른 개발자가 보고 "실수로 누락했나?"라는 오해를 하지 않을까 걱정된다.

이에 대한 해결책으로 고안해낸 것이 ~~`equals`를 재정의 하되, `Object`의 것과 그 로직을 똑같이 하고 주석으로 그 이유를 명시해주는 것이다.~~

위 해결책을 적용하려고 막상 보니까 괜히 `Lotto`에 필요 없는 코드를 추가함으로 인한 가독성 저하가 우려된다.

그리고 `equals`를 재정의 하는 이유는 보통 논리적 동등성을 비교하기 위함인데, (물론 구현 코드까지 보러 들어 오면 그 의도를 파악하겠지만)
그냥 재정의 된 것만 보면 "아, 얘는 논리적 동등성 비교 하겠구나. 생성할 때 `numbers`만 전달하니까 이거 기반으로 하겠지?"라는 착각을 할 위험도 있겠다.

`how to specify information for java` 키워드로 검색하니 익숙한 Google Java Style Guide가 상위에 뜬다.

여기서 소개하는 Javadoc을 사용함이 적절하겠다!!!

### 로또 게임과 로또 발매기
기능 요구 사항 중 다음 두 표현이 있다.
- "간단한 로또 발매기를 구현한다."
- "... 로또 게임을 종료한다."

이에 대해서 '로또 발매기가 로또 게임을 진행하는 책임까지 가지는 것'보다 '로또 게임의 요소중 로또 발매기'가 있는 것이 적절하다고 판단한다.

예를 들어, 로또 발매기는 이름 그대로 '로또 발매'의 책임만 가지면 된다. '당첨 번호와 보너스 번호를 입력받고, 당첨을 계산하는' 책임 등은 '발매기'가 아니라, '게임'의 요소라고 판단함이 옳다.

이에 따라 로또 발매기는 `LottoVendingMachine` 객체로 구현하여 인자로 받은 금액만큼의 로또를 발행하는 책임을 가지도록 한다.
- 구입시에는 구입할 금액을 전달한다.
- `구입`은 `purchase`로 명명한다. '발행'에 초점을 맞춘다면 'issue'라는 단어를 사용할 수 있지만, 해당 객체와 소통하는 다른 객체의 입장에서 보면 '구입한다'는 의미의 `purchase`가 적절하다.
  - 'buy'도 고려했으나, 구입할 금액에 대한 변수명을 짓기가 적절하지 않다.
  - `구입할 금액`은 `purchaseAmount`로 명명한다. price는 어떤 제품의 가격에 쓸 만한 표현이지, 구입하려고 내미는 금액에는 적절하지 않다는 판단이다.
- 구입 금액에 대해서 몇 개의 로또를 발행해야 하는지 계산할 수 있어야 하기 때문에, `로또 가격`을 알아야 한다. 이에 대해서는 `PRICE`라는 정적 상수를 사용한다.
- 구입을 요청한 특정 객체에게 발행해 준 로또를 로또 발매기가 상태로 관리할 이유가 없다. 요청에 따른 로또(들)을 리턴해주는 것에서 책임이 종료된다.
  - 즉, 로또 발매기는 개별 인스턴스가 관리해야 할 필드가 존재하지 않기 때문에 정적 유틸 클래스로 한다.

### 로또 게임
로또 게임은 애플리케이션과 직접 소통하는 진입점 역할을 한다.

애플리케이션은 로또 게임을 통해 구입 금액을 입력하고, 구매 내역을 확인하고, 당첨/보너스 번호를 입력받고, 당첨 통계와 수익률을 얻을 수 있다.

즉, 사용자는 로또 게임만 알면 되고 그 이후의 일은 로또 게임이 알아서 잘 해주도록 하자.

로또 게임이 조율할 여러 책임 중 아직 구현하지 못한 부분이 있다. 일단 그 중에서 당첨 번호와 보너스 번호를 입력받는 책임을 적절한 객체(`InlineView`)에게 위임하자.

#### InputView
이 객체는 사용자로부터 입력해 달라는 프롬프트를 출력하고, 값을 입력받는 책임을 가진다.

먼저, 미션에서 제공되는 `Console` 라이브러리가 입력을 문자열로 받는 `readLine`만 제공하기 때문에, 내부에서 형변환을 거쳐 정수로 받을 수 있게끔 하는 기능을 추가하고,
외부 라이브러리에 대한 의존성을 래핑해서 사용할 수 있도록 다음 두 메서드를 구현하자.
- `String readLine()`: `Console.readLine()`을 그대로 래핑한다.
- `int readLineAsInt()`: 내부적으로 형변환을 거쳐 `int`로 리턴한다.

그리고 당첨 번호와 보너스 번호를 입력해달라는 프롬프트 출력과 실제로 입력받는 기능의 구현 메서드는 다음과 같이 네이밍 한다.
- `readWinningNumbers`
- `readBonusNumber`

네이밍은 구글 검색을 통해 다음을 참고했다. (regular numbers와 extra number라는 표현도 사용 가능함~~으로 보이지만 이름의 직관성이 부족하다고 생각한다.~~)

![winning-numbers](https://github.com/user-attachments/assets/59de43f1-a492-4fd6-ac7b-95f61807d4ce)

### 당첨 통계를 내는 책임
당첨 통계는 당첨, 보너스 번호를 기준으로 구매한 로또들이 몇 개나 맞췄는지 비교해서 낼 수 있다.

이때 구매한 로또들은 `LottoVendingMachine`을 통해 얻을 수 있다.

당첨, 보너스 번호는 `InputView`로 입력받을 수 있지만, 이를 보관할 객체는 마땅치 않다.

따라서 새롭게 설계할 어떤 객체가 당첨, 보너스 번호를 상태로 가지거나 또는 로또들과 함께 입력받아서 당첨 통계를 계산할 수 있어야 한다.

validate -> validator, produce -> producer처럼 statistics -> statisticor...?으로 객체 네이밍을 하려 했으나 적절한 표현이 존재하지 않는다.

~~대안으로 `LottoStatisticsCalculator`로 네이밍 하기로 한다.~~

~~(기존 네이밍이 뭔가 Lotto 자체에 종속된 느낌이라 다음과 같이 바꾼다. Lotto 자체로는 어떤 통계량을 계산할 수 없고, 내가 계산하려는 통계량은 '로또 게임의 통계량'이기 때문!)~~

~~`LottoGameStatisticsCalculator`~~

(막상 구현하고 나니, 로또 게임의 여러 기능 중 하나가 당첨 통계 계산일 뿐이라는 생각으로 바뀌었다. 즉, '로또 게임에 대한 통계'가 아니고, '당첨 통계'라고 부름이 더 적절하다는 것이다.
이에 따라 `LottoGameStatisticsCalculator`와 `LottoGameStatisticDto`의 접두사를 `LottoGame`에서 `Winning`으로 변경한다.)

해당 객체가 어떤 값을 뱉어내야 되는지 판단하기 위해서는 결국 "최종 출력을 위해 어떤 정보가 필요한지"를 고민하면 되겠다.
- 각 당첨 순위에 해당하는 로또의 수
- 총 당첨 금액
- 총 로또 구매 금액

이때 위 정보들은 결국 '로또 통계량'이라는 큰 범주로 묶을 수 있겠다.

결국 당첨 통계라는 하나의 목적을 위해 리턴해야 되는 값들이기 때문에 별도의 DTO로 관리하면 좋겠다.

또한 위 정보들을 제공하기 위해서는 다음 정보들이 필요하다.
- 당첨 번호
- 보너스 번호
- 구매한 로또 리스트

#### 당첨 번호와 보너스 번호
당첨 번호와 보너스 번호는 같이 관리되어야 함이 적절하다. 왜냐하면 서로 간에 중복이 발생하면 안 되기 때문이다.

`LottoNumber`로 이루어지고, 중복이 발생하면 안 된다는 점이 `Lotto`와 유사해 보이지만, 하나 차이가 있다.

로또는 6개의 번호만을 포함하지만, 당첨 로또(?)는 7개의 번호를 포함한다는 것이다. 즉, 당첨 번호와 보너스 번호를 `Lotto`에서 관리할 수는 없다.

"당첨 번호와 보너스 번호를 포함해서 일컫는 단어가 뭐지?"라는 물음에 답을 찾을 수 없어 실사례를 확인하기 위해 네이버에 '로또 당첨 번호'를 검색했다.

![당첨-번호에는-메인-넘버와-보너스-넘버가-포함된다](https://github.com/user-attachments/assets/0a8f1285-5522-452b-9b2d-d6752625bcdc)

~~실사례를 보니 당첨 번호라는 표현은 메인 넘버와 보너스 넘버를 모두 포함하는 것으로 봐야 옳겠다..!! 이에 따라 [InputView](#InputView)의 네이밍도 변경한다.~~

입출력 예시에서 '당첨 번호'라는 표현일 때 6개의 번호만 입력하는 시나리오미므로, 당첨 번호 = 6개 + 보너스 번호라고 판단함은 적절하지 않겠다는 것이 최종 판단이다. (네이밍이 너무 혼란스러워서 자주 바꾸게 된다 ㅠㅠ)

당첨 번호 6개와 보너스 번호 1개를 포함하기 위한 객체는 그냥 '당첨 로또' 즉, `WinningLotto`라고 하자!!!

## 3주 차 목표
- [x] 코드 컨벤션 중 class 선언부 다음에 빈 줄을 두지 말라는 컨벤션을 지킨다. (아래 `intellij-java-wooteco-style.xml` 중 일부 참고)
    ```xml
    <option name="BLANK_LINES_AFTER_CLASS_HEADER" value="0"/>
    ```
- [ ] 하나의 테스트 클래스에서 테스트 케이스가 다양하다면, `@Nested` 애노테이션으로 구분하기
- [ ] 스트림 내부에서의 예외 처리가 가능함을 인지하고, 필요한 경우 적용하기
- [x] 커밋에 깃모지 사용하기
- [ ] 메서드를 접근 제어자가 아니라, 기능 별(논리적 단위)로 묶기
- [x] 커밋 메시지 컨벤션을 지킨다.
  - 2주 차까지는 제목 50자, 본문 72자 제한이 '글자 수' 기준인 줄 알고, 인크루트 글자 수 검사기로 체크했다...!
  - 3주 차 진행 중에 갑자기 의문이 들어 찾아 보니 화면에 표시되는 칸 수 기준이었고, 기존의 내 커밋 제목들은 이를 한참 초과해버렸다.
  - vi 에디터를 사용중인데, [Configuring Git and Vim](https://csswizardry.com/2017/03/configuring-git-and-vim/)을 참고해서 50번 째와 72번 째 컬럼에 색을 넣어 표시해주는 설정을 해서 사용하기로 한다.

## References
`STUDY_NOTE`에 출처를 기재한 경우 여기서는 생략될 수 있다.

[VO(Value Object)는 무엇일까? 왜 사용할까?](https://ksh-coding.tistory.com/83)

[방어적 복사를 한 변수의 네이밍](https://stackoverflow.com/questions/39139885/how-to-name-a-variable-that-is-a-copy-of-a-parameter)

[Arrays.stream()과 Stream.of()](https://medium.com/@javageeksociety/arrays-stream-vs-stream-of-in-java-8-be7d6b757e4e)
